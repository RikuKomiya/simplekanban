...
## Issue orchestration smoke flow

SimpleKanban issue を外部 orchestrator から処理するときは、最小 smoke check として次を確認する。

- `kan auth status --json` で認証と API URL を確認する
- `kan issue view <identifier> --json --comments` で現在 state と既存 Workpad comment を読む
- `Todo` の issue は `In Progress` に遷移してから `## Codex Workpad` comment を作成または更新する
- Workpad には plan、acceptance criteria、validation、notes、confusions を残し、検証コマンドと結果を記録する
- 完了前に `bun run build && bun run typecheck && bun run test` と PR feedback sweep の結果を Workpad に反映する
