...
API key は workspace-scoped かつ permission scope 付き。未指定/既存keyは `*` (full access) として扱う。coding agent 向けの代表scopeは `issues:read`, `issues:write`, `comments:read`, `comments:write`, `states:read`, `members:read`, `relations:read`, `relations:write`, `usage:write`。

レスポンス形: 成功 `{ data: T }`、リスト `{ data: T[] }`、ページ付きリスト `{ data: T[], pageInfo: { hasNextPage, nextCursor } }`、エラー `{ error: { code: string, message: string } }`。HTTP status 準拠 (400/401/403/404/409/500)。
...
GET    /api/v1/teams/:teamId/issues        ?state=&assignee=&priority=&label=&cycle=&project=&q=&updatedSince=&limit=&cursor=
POST   /api/v1/teams/:teamId/issues        {title, description?, stateId?, priority?, assigneeId?, labelIds?, ...}
POST   /api/v1/issues/batch                {ids: [...]} → visible issues in requested order
...
GET/POST /api/v1/issues/:id/comments       / PATCH/DELETE /api/v1/comments/:id
GET    /api/v1/issues/:id/activity         issue activity timeline
GET/POST /api/v1/issues/:id/blockers       {blockerIssueId} / DELETE .../blockers/:blockerIssueId
POST   /api/v1/issues/:id/usage            {tokens} → cumulative token usage
